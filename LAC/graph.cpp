...
using namespace std;
Graph::Graph(int blockSizeVertex, int blockSizeEdge, int cachePercent)
    : blockSizeVertex(blockSizeVertex), blockSizeEdge(blockSizeEdge), cachePercent(cachePercent) {}
...
void Graph::loadDataToStorage(const char* fileName, int policy) {
  ...
  data = new Storage(nodeData, edges, blockSizeVertex, blockSizeEdge);
  data->loadData(edges, nodeData);
  
  int cacheCapacity = ((data->getNumEdgeBlocks() + data->getNumNodeBlocks()) * cachePercent) / 100;
  cacheCapacity += ((data->getNumEdgeBlocks() + data->getNumNodeBlocks()) * cachePercent) % 100 == 0 ? 0 : 1;
  //printf("number of page on disk %d, number of page in cache %d\n", data->getNumBlocks(), cacheCapacity);

  if(policy == 1){
    cache = new LACGCCache(data, cacheCapacity, blockSizeVertex, blockSizeEdge, (data->getNumEdgeBlocks() + data->getNumNodeBlocks()));
  }
  else if (policy == 2){
    cache = new GClockCache(data, cacheCapacity, blockSizeVertex, blockSizeEdge, data->getNumEdgeBlocks());
  }
...
int Graph::bidiSP(int src, int dst) {
  ...
  while (!srcq.empty() || !dstq.empty()) {
    ...
    if (intersect.size() > 1 || (intersect.size() == 1 && *(intersect.begin()) != src)) {
      return time;
    }
    level++;
