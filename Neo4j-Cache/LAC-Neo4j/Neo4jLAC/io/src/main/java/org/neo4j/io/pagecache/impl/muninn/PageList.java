...
class PageList implements PageReferenceTranslator {
    ...
    private static SwapperSet swappers; //this was final
    ...
    static void incrementUsage(long pageRef) {
        ...
        PageSwapper swapper = swapperMapping.swapper;
         ...
         if(swapper.path().endsWith("neostore.nodestore.db") && PageCursor.diskPagesNode.length>1){
            long filePageID = getFilePageId(pageRef);
            double distance = getPageMinDistanceNodeDisk(filePageID);
            distance = (1-(distance/MAX_DIST));
            PageCursor.diskPagesNode[(int)filePageID] = (float)Math.min(PageCursor.diskPagesNode[(int)filePageID]+distance, (double)MAX_USAGE_COUNT);
        }
        else if(swapper.path().endsWith("neostore.propertystore.db") && PageCursor.diskPagesNode.length>1){
            long filePageID = getFilePageId(pageRef);
             double distance = getPageMinDistancePropertyDisk(filePageID);
             distance = (1-(distance/MAX_DIST));
             PageCursor.diskPagesProp[(int)filePageID] = (float)Math.min(PageCursor.diskPagesProp[(int)filePageID]+distance, (double)MAX_USAGE_COUNT);
        }
        else if(swapper.path().endsWith("neostore.relationshipstore.db") && PageCursor.diskPagesNode.length>1){
             long filePageID = getFilePageId(pageRef);
             double distance = getPageMinDistanceDisk(filePageID);
             distance = (1-(distance/MAX_DIST));
             PageCursor.diskPagesRel[(int)filePageID] = (float)Math.min(PageCursor.diskPagesRel[(int)filePageID]+distance, (double)MAX_USAGE_COUNT);
         }
        else{
     ...
     static boolean decrementUsage(long pageRef) {
        ...
        PageSwapper swapper = swapperMapping.swapper;
         ...
         if(swapper.path().endsWith("neostore.nodestore.db") && PageCursor.diskPagesNode.length>1){
             ...
             PageCursor.diskPagesNode[(int)filePageID] = (float)Math.max(PageCursor.diskPagesNode[(int)filePageID]-distance, 0);

             return PageCursor.diskPagesNode[(int)filePageID] <= 1;
        }
        else if(swapper.path().endsWith("neostore.propertystore.db") && PageCursor.diskPagesNode.length>1){
            long filePageID = getFilePageId(pageRef);
            double distance = getPageMinDistancePropertyDisk(filePageID);
            distance = distance/MAX_DIST;
            PageCursor.diskPagesProp[(int)filePageID] = (float)Math.max(PageCursor.diskPagesProp[(int)filePageID]-distance, 0);

            return PageCursor.diskPagesProp[(int)filePageID] <= 1;
        }
        else if(swapper.path().endsWith("neostore.relationshipstore.db") && PageCursor.diskPagesNode.length>1){
            long filePageID = getFilePageId(pageRef);
            double distance = getPageMinDistanceDisk(filePageID);
            distance = distance/MAX_DIST;
            PageCursor.diskPagesRel[(int)filePageID] = (float)Math.max(PageCursor.diskPagesRel[(int)filePageID]-distance, 0);

            return PageCursor.diskPagesRel[(int)filePageID] <= 1;
         }
        else{
            long address = offPageBinding(pageRef);
            long value = UnsafeUtil.getLongVolatile(address);
    ...
    static double getPageMinDistanceDisk(long filePageID){
        long leftDist=Long.MAX_VALUE;
        long rightDist=Long.MAX_VALUE;
        int curr = (int)filePageID;
        int i = curr+1;
        int maxI = Math.min(PageCursor.diskPagesRel.length - 1, curr + MAX_DIST);
        while(i<=maxI){
            if(PageCursor.diskPagesRel[i]!=0){
                rightDist = i-filePageID;
                break;
            }
            i++;
        }
        i = curr-1;
        int minI = Math.max(0, curr - MAX_DIST);
        while(i>=minI){
            if(PageCursor.diskPagesRel[i]!=0){
                leftDist = filePageID-i;
                break;
            }
            i--;
        }
        return Math.min(Math.min(leftDist,rightDist), MAX_DIST);

    }

    static double getPageMinDistanceNodeDisk(long filePageID){
        long leftDist=Long.MAX_VALUE;
        long rightDist=Long.MAX_VALUE;
        int curr = (int)filePageID;
        int i = curr+1;
        int maxI = Math.min(PageCursor.diskPagesNode.length - 1, curr + MAX_DIST);
        while(i<=maxI){
            if(PageCursor.diskPagesNode[i]!=0){
                rightDist = i-filePageID;
                break;
            }
            i++;
    ...
    static double getPageMinDistancePropertyDisk(long filePageID){
        long leftDist=Long.MAX_VALUE;
        long rightDist=Long.MAX_VALUE;
        int curr = (int)filePageID;
        int i = curr+1;
        int maxI = Math.min(PageCursor.diskPagesProp.length - 1, curr + MAX_DIST);
        while(i<=maxI){
            if(PageCursor.diskPagesProp[i]!=0){
                rightDist = i-filePageID;
                break;
            }
            i++;
    ...
    private void evict(long pageRef, EvictionEvent evictionEvent) throws IOException {
        ...
        if (swapperId != 0) {
            ...
            if (swapperMapping != null) {
                ...
                if (swapper.path().endsWith("neostore.nodestore.db") && PageCursor.diskPagesNode.length>1) {
                    int i = (int) getFilePageId(pageRef);
                    PageCursor.diskPagesNode[i] = 0;
                }
                else if(swapper.path().endsWith("neostore.propertystore.db") && PageCursor.diskPagesNode.length>1){
                    int i = (int) getFilePageId(pageRef);
                    PageCursor.diskPagesProp[i] = 0;
                }
                else if (swapper.path().endsWith("neostore.relationshipstore.db") && PageCursor.diskPagesNode.length>1) {
                    int i = (int) getFilePageId(pageRef);
                    PageCursor.diskPagesRel[i] = 0;
                }
            }
        }
