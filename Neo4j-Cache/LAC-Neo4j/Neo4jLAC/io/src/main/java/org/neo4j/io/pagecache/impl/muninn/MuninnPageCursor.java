        ...
        for (; ; ) {
            ...
            if (mappedPageId != UNMAPPED_TTE) {
                ...
                if (locked && PageList.isBoundTo(pageRef, swapperId, filePageId)) {
                    ...
                    pinEvent.hit();
//                    PageCursor.tracer.hits(1);//FAULT
//                    if(swapper.path().endsWith("neostore.relationshipstore.db") ){
//                        PageCursor.relHit ++;
//                    }
//                    else if(swapper.path().endsWith("neostore.nodestore.db") ){
//                        PageCursor.nodeHit ++;
//                    }
//                    else{
//                        PageCursor.otherHit ++;
//                    }
    ...
    private boolean uncommonPin(PinEvent pinEvent, long filePageId, int chunkIndex, int[] chunk) throws IOException {
        ...
        PageCursor.tracer.faults(1); //FAULT
        if(swapper.path().endsWith("neostore.relationshipstore.db")){
            PageCursor.relMiss ++;
        }
        else if(swapper.path().endsWith("neostore.nodestore.db")){
